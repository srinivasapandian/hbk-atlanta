...
function SectionSkeleton({ type }) {
  if (type === 'footer') {
    return (
      <div className="section-skeleton section-skeleton-footer" aria-hidden="true">
...
export default function Home() {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (key) setLoaded((prev) => (prev[key] ? prev : { ...prev, [key]: true }));
        });
      },
  ...
  useEffect(() => {
    const target = location.state?.scrollTo || sectionRouteMap[location.pathname] || 'home';
    const lazyKey = lazyMap[target];
    if (lazyKey) setLoaded((prev) => (prev[lazyKey] ? prev : { ...prev, [lazyKey]: true }));

    let attempts = 0;
    function tryScroll() {
      const el = document.getElementById(target);
      if (el) {
        el.scrollIntoView({ behavior: target === 'home' ? 'auto' : 'smooth', block: 'start' });
        return;
      }
      if (attempts < 240) { attempts++; requestAnimationFrame(tryScroll); }
    }
    requestAnimationFrame(tryScroll);
  }, [location.pathname, location.state?.scrollTo, location.state?.scrollRequestId]);
